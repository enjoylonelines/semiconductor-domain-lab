"""Show whether process-local OpenSTA budgets constrain independent submitters."""
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="eda-cross-budget-") as directory_name:
        ...
        try:
            for index in range(CONTENDERS):
                ...
                process = subprocess.Popen([
                    ...
                    "--child-pids", str(child_pids), "--worker-id", f"contender-{index}", "--job-id", f"cross-budget-{index}",
                    "--max-in-flight", str(CAPACITY),
            ...
            print(json.dumps({
                "synthetic": False,
                "scope": f"{CONTENDERS} external services each configured with max_in_flight={CAPACITY}",
                "configured_process_local_budget": CAPACITY,
