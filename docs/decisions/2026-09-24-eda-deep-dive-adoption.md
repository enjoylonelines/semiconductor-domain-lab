...
`JobService`에서 같은 profile(프로파일)로 8개 actual OpenSTA(실제 OpenSTA) 작업을 실행했을 때 9번째 submit(제출)은 Run(작업)을 만들지 않고 `BackpressureError`로 거절됐고, 8개 완료 뒤 다음 작업은 성공했다. external service(외부 서비스)가 여러 process(프로세스)로 나뉘면 기존 process-local budget(프로세스 로컬 예산)만으로는 9번째 실행을 막지 못한다는 기준선도 재현됐다. 따라서 SQLite(라이트급 SQL 저장소) `BEGIN IMMEDIATE` transaction(즉시 트랜잭션)으로 same-host(동일 호스트) shared admission budget(공유 입장 예산)을 추가했고, 같은 9개 외부 제출 재검증에서 8개만 실제 child process(하위 프로세스)를 실행하고 하나는 저장 없이 거절됐다. [교차 프로세스 예산 근거](../evidence/2026-09-25-opensta-cross-process-budget.md)에 따라 이 profile(프로파일)의 현재 admission policy(입장 정책)는 persistent queue(영속 대기열)가 아닌 bounded rejection(제한된 거절)이다. retry delay(재시도 지연), fairness(공정성), priority queue(우선순위 대기열), sustained arrival rate(지속 도착률)는 아직 측정하지 않았으며, 이 근거를 넘는 주장으로 승격하지 않는다.
...
호출자는 같은 `job_id`를 보존하고 server hint(서버 힌트) 뒤 bounded retry(제한 재시도)와 jitter(무작위 지연)를 적용해 재제출한다. 한 요청 흐름의 backpressure retry(역압 재시도)는 최대 3회로 제한하고, 이후에는 호출자에게 명시 실패를 돌려준다. server(서버)는 거절된 Run(작업)을 저장하거나 내부 대기열에 넣지 않는다. `job_id`와 canonical spec hash(정규 명세 해시)가 같을 때만 기존 Run(작업)을 반환한다. 같은 `job_id`에 다른 명세를 보내면 HTTP(하이퍼텍스트 전송 프로토콜) `409`, `retryable=false`로 거절한다.
